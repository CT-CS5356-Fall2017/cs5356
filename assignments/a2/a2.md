...
[Auto-grader for the REST API](grade_a2.py) is up. This time with python (with the hope that will work accross all platforms). 
```bash
$ python grade_a2.py <your-url>
```

`<your-url>` should be your server url (without the angle brackets), e.g., `http://localhost:8080`. You might need to install `requests` library, which
can be installed using `pip` (Python package manager, https://pip.pypa.io/en/stable/installing/): `pip install requests`. 
